...
public class Battlefield : MonoBehaviour {
		...
		if(enable_update)
		{
			...
			if(recordManager.frameData[cf].bulletData != null)
			{
				foreach(RecordManager.BulletData b in recordManager.frameData[cf].bulletData)
				{
					...
					float h = Robot.BULLET_HEIGHT[b.bulletType];
					...
					else
					{
						...
						if(b.launcherID < 0)
						{
							...
							//no launcherID
							bullet_height = Robot.BULLET_HEIGHT[b.bulletType] + BULLET_HEIGHT_OFFSET;
						}
						else
						{
	...
	void InitBattleMap()
	{
		...
		foreach(RecordManager.ObstacleData o in recordManager.mapData.obstacleData)
		{
			...
			float size_radium = o.r / ( obstacles[i].renderer.bounds.size.x / 2 );
		...
		{
			robots[j] = new Robot(j,r.weaponTypeID,r.engineTypeID
